...
export class PropertiesPanel {
  ...
  render(state) {
    ...
    if (selectedIds.length > 1) {
      ...
      const colorValue = firstClip.color || '#3f7182';
      ...
      const colorMixed = !allSame(clip => clip.color || '#3f7182');
    ...
    if (clip) {
      ...
      <div class="property-group">
        ...
        <input type="color" class="color-picker" id="${idPrefix}-color" value="${clip.color || '#3f7182'}">
      </div>
    ...
    if (builderCopyBtn) {
      builderCopyBtn.addEventListener('click', () => {
        ...
        const baseColor = clip ? clip.color : '#3f7182';
        let insertTime = 0;
        let inserted = 0;
        const clipsToCopy = [];
